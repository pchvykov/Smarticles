  ...
  [9])
{
  ...
  b_repmat(b_XY2, X3);
  for (i2 = 0; i2 < 3; i2++) {
    b_XY2[i2] = c_XY2[1 + (i2 << 2)];
  }

  b_repmat(b_XY2, Y3);
  for (i2 = 0; i2 < 3; i2++) {
    b_XY2[i2] = c_XY2[2 + (i2 << 2)];
  }

  b_repmat(b_XY2, X4_X3);
  for (i2 = 0; i2 < 9; i2++) {
    Y1_Y3[i2] = Y1[i2] - Y3[i2];
    X4_X3[i2] -= X3[i2];
  }

  for (i2 = 0; i2 < 3; i2++) {
    b_XY2[i2] = c_XY2[3 + (i2 << 2)];
  }
  ...
  for (i2 = 0; i2 < 9; i2++) {
    b_Y3 = Y3[i2] - X1[i2];
    b_Y2_Y1 = Y2_Y1[i2] - Y1[i2];
    denominator = Y4_Y3[i2] * b_Y3 - X4_X3[i2] * b_Y2_Y1;
    b_X4_X3 = (X4_X3[i2] * Y1_Y3[i2] - Y4_Y3[i2] * X3[i2]) / denominator;
    denominator = (b_Y3 * Y1_Y3[i2] - b_Y2_Y1 * X3[i2]) / denominator;
    INT_B = ((b_X4_X3 >= 0.0) && (b_X4_X3 <= 1.0) && (denominator >= 0.0) &&
             (denominator <= 1.0));
    out_intAdjacencyMatrix[i2] = INT_B;
    out_intMatrixX[i2] = (X1[i2] + b_Y3 * b_X4_X3) * (real_T)INT_B;
    out_intMatrixY[i2] = (Y1[i2] + b_Y2_Y1 * b_X4_X3) * (real_T)INT_B;
    out_intNormalizedDistance1To2[i2] = b_X4_X3;
    out_intNormalizedDistance2To1[i2] = denominator;
  }
